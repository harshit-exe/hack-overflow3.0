...
import { AlertCircle, Maximize, Minimize, Settings, Download, Save, BarChart, Trash2 } from "lucide-react"
...
export default function CareerGuidanceInterviewSimulator() {
  ...
  // Custom hooks
  const { generateQuestion, evaluateAnswer, generateCareerAdvice, isLoading, error: aiError } = useGroqAI()
    ...
    const loadSavedData = () => {
      try {
        ...
        if (savedSettings) {
          ...
          if (settings.useAIVoice !== undefined) setUseAIVoice(settings.useAIVoice)
          if (settings.showTimer !== undefined) setShowTimer(settings.showTimer)
          if (settings.interviewDuration) setInterviewDuration(settings.interviewDuration)
          if (settings.detailedFeedback !== undefined) setDetailedFeedback(settings.detailedFeedback)
        ...
        // Load career paths
        const savedCareerPaths = localStorage.getItem("careerGuidanceCareerPaths")
        if (savedCareerPaths) {
          setCareerPaths(JSON.parse(savedCareerPaths))
        }

        // Load saved sessions
        const savedSessionsData = localStorage.getItem("careerGuidanceSavedSessions")
  ...
  const stopAnswering = useCallback(async () => {
    ...
    try {
      const evaluation = await evaluateAnswer(currentQuestion, transcript, detailedFeedback)
  ...
  const nextQuestion = useCallback(async () => {
    ...
    try {
      ...
      const question = await generateQuestion(selectedCareerPath, difficultyLevel)
  ...
  }, [selectedCareerPath, generateQuestion, useAIVoice, isIntroduction, speakWithTracking, difficultyLevel])
  ...
  const addNewCareerPath = useCallback(() => {
    if (newCareerPath.trim() !== "") {
      ...
      localStorage.setItem("careerGuidanceCareerPaths", JSON.stringify(updatedPaths))
    }
  ...
  const generateHint = useCallback(async () => {
    ...
    try {
      const messages = [
        { role: "system", content: "You are an AI assistant that provides helpful hints for interview questions." },
        {
          role: "user",
  ...
  const handleGenerateAdvice = useCallback(
    async (careerPath, educationLevel, strengths, weaknesses) => {
      ...
      try {
        const advice = await generateCareerAdvice(careerPath, educationLevel, strengths, weaknesses)
  ...
  }, [currentQuestion, isAnswering, transcript, userAnswer, feedback, careerFitScore])
  ...
  const saveSession = useCallback(() => {
    ...
    // Save to localStorage
    localStorage.setItem("careerGuidanceSavedSessions", JSON.stringify(updatedSessions))
  ...
  const exportData = useCallback(() => {
    ...
    const exportFileDefaultName = `career-guidance-data-${new Date().toISOString().slice(0, 10)}.json`
  ...
  const saveSettings = useCallback(() => {
    ...
    localStorage.setItem("careerGuidanceSettings", JSON.stringify(settingsToSave))
    alert("Settings saved successfully!")
  }, [useAIVoice, showTimer, interviewDuration, detailedFeedback, rate, pitch, currentLanguage])

  const importData = useCallback(
    (data) => {
      try {
        if (data.settings) {
          if (data.settings.useAIVoice !== undefined) setUseAIVoice(data.settings.useAIVoice)
          if (data.settings.showTimer !== undefined) setShowTimer(data.settings.showTimer)
          if (data.settings.interviewDuration) setInterviewDuration(data.settings.interviewDuration)
          if (data.settings.detailedFeedback !== undefined) setDetailedFeedback(data.settings.detailedFeedback)
        ...
        saveSettings()
        localStorage.setItem("careerGuidanceCareerPaths", JSON.stringify(data.careerPaths || careerPaths))
        localStorage.setItem("careerGuidanceSavedSessions", JSON.stringify(data.savedSessions || savedSessions))
  ...
  const extractCareerFitScore = (evaluation) => {
    ...
    const positiveTerms = ["excellent", "great", "good", "strong", "impressive", "thorough", "comprehensive", "well"]
    const negativeTerms = ["poor", "weak", "inadequate", "insufficient", "lacks", "missing", "improve", "limited"]
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold text-[#ffffff] text-center mb-4">CAREER GUIDANCE ASSISTANT</h1>
          ...
          <Card className="max-w-2xl mx-auto bg-black text-white border-[#6366F1] border-2">
            ...
            <CardContent>
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="flex items-center space-x-2">
                  <Switch id="detailed-feedback" checked={detailedFeedback} onCheckedChange={setDetailedFeedback} />
                  <Label htmlFor="detailed-feedback" className="text-sm font-medium text-gray-300">
                    Detailed Feedback
                  </Label>
                </div>
                <Button
                ...
                {savedSessions.length > 0 && (
                  <div className="w-full mt-4">
                    <h3 className="text-lg font-semibold text-[#6366F1] mb-2">Previous Sessions</h3>
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              {new Date(session.timestamp).toLocaleDateString()} • {session.careerPath}
                            </p>
                          </div>
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                const dataUri = `data:application/json;charset=utf-8,${encodeURIComponent(dataStr)}`
                                const exportFileName = `${session.name.replace(/\s+/g, "-")}-${new Date(session.timestamp).toISOString().slice(0, 10)}.json`
          ...
          <div className="flex-grow overflow-hidden">
            <Tabs value={currentTab} onValueChange={setCurrentTab} className="h-full flex flex-col">
                          ...
                          <div className="mt-4">
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex gap-2">
                                ...
                                />
                                <Button onClick={saveSession} size="sm" className="h-8 bg-[#6366F1] hover:bg-[#4F46E5]">
              ...
              <TabsContent value="history" className="flex-grow overflow-auto">
                <Card className="bg-gray-900 text-white border-[#6366F1] border-2">
                  <CardHeader className="flex flex-row items-center justify-between">
                    <CardTitle className="text-[#6366F1]">Session History</CardTitle>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        ...
                        onClick={() => {
                          ...
                          const dataUri = `data:application/json;charset=utf-8,${encodeURIComponent(dataStr)}`
                          const exportFileName = `career-guidance-session-${new Date().toISOString().slice(0, 10)}.json`
                  ...
                  <CardContent className="p-4">
                    {interviewHistory.length === 0 ? (
                      <div className="text-center py-8 text-gray-400">
                        <p>No history yet. Complete some questions to see them here.</p>
                      </div>
                    ) : (
              ...
              <TabsContent value="report" className="flex-grow overflow-auto">
                <Card className="bg-gray-900 text-white border-[#6366F1] border-2">
                  <CardHeader>
                    <CardTitle className="text-[#6366F1]">Performance Report</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {interviewHistory.length === 0 ? (
                      <div className="text-center py-8 text-gray-400">
                      ...
                      <div className="space-y-6">
                        <div className="bg-gray-800 p-4 rounded-lg">
                          <h3 className="text-lg font-semibold mb-4 text-[#57FF31]">Session Overview</h3>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <p className="text-sm text-gray-400">Career Path</p>
                              <p className="text-lg">{selectedCareerPath.replace(/-/g, " ")}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-lg">{interviewHistory.length}</p>
                            </div>
                            <div>
                              <p className="text-sm text-gray-400">Average Score</p>
                              <p className="text-lg">{overallPerformance.toFixed(2)}%</p>
                            </div>
                          </div>
                        </div>

                        <div className="bg-gray-800 p-4 rounded-lg">
                          <h3 className="text-lg font-semibold mb-4 text-[#57FF31]">Performance Breakdown</h3>
                          <div className="space-y-4">
                            {interviewHistory.map((item, index) => (
                              <div key={index} className="flex items-center">
                                ...
                                <div className="flex-1">
                                  <div className="flex justify-between mb-1">
                                    <span className="text-sm truncate max-w-[200px]">{item.question}</span>
                                    <span className="text-sm font-bold">{item.score.toFixed(0)}%</span>
                                  </div>
                                  <Progress
                                    value={item.score}
                        ...
                        <div className="bg-gray-800 p-4 rounded-lg">
                          <h3 className="text-lg font-semibold mb-4 text-[#57FF31]">Recommendations</h3>
                          <p className="text-gray-300 mb-4">
                            Based on your performance, here are some recommendations to improve your career readiness:
                          </p>
                          <ul className="list-disc pl-5 space-y-2 text-gray-300">
                            {overallPerformance < 60 && (
                              <>
                                <li>
                                  Focus on building a stronger understanding of core concepts in{" "}
                                  {selectedCareerPath.replace(/-/g, " ")}.
                                </li>
                                <li>Consider taking introductory courses to strengthen your foundation.</li>
                              </>
                            )}
                            {overallPerformance >= 60 && overallPerformance < 80 && (
                              <>
                                <li>
                                  You have a good foundation. Consider intermediate-level courses to advance your
                                  knowledge.
                                </li>
                                <li>Practice articulating your thoughts more clearly in interview settings.</li>
                              </>
                            )}
                            {overallPerformance >= 80 && (
                              <>
                                <li>Your performance is excellent! Consider advanced specialization in your field.</li>
                                <li>Focus on building a portfolio that showcases your expertise.</li>
                              </>
                            )}
                            <li>Continue practicing with different question types to improve versatility.</li>
                            <li>
                              Review the feedback provided for each question to identify specific areas for improvement.
                            </li>
                          </ul>
                        </div>
