...
export default function ProjectRecommenderPage() {
  ...
  const [trendingTechnologies, setTrendingTechnologies] = useState([
    {
      name: "AI/ML",
      trend: "rising",
      description: "Artificial Intelligence and Machine Learning continue to be in high demand",
    },
    { name: "Web3", trend: "stable", description: "Blockchain and decentralized applications remain relevant" },
    {
      name: "Cloud Native",
      trend: "rising",
      description: "Kubernetes, microservices, and serverless architectures are growing",
    },
    {
      name: "DevOps",
      trend: "rising",
      description: "CI/CD, Infrastructure as Code, and automation skills are highly valued",
    },
    {
      name: "Mobile Development",
      trend: "stable",
      description: "React Native and Flutter are popular for cross-platform development",
    },
  ])
  ...
  useEffect(() => {
    try {
      ...
      if (savedData) {
        ...
        setStatusMessage({
          type: "warning",
          message: "No resume data found. Please create a resume first for personalized recommendations.",
        })
      }
  ...
  useEffect(() => {
    if (savedProjects.length > 0) {
      localStorage.setItem("saved-project-ideas", JSON.stringify(savedProjects))
    }
  }, [savedProjects])
  ...
  const handleGenerateProjects = async () => {
    ...
    if (!jobRole) {
      setStatusMessage({
        type: "warning",
        message: "Please enter your target job role for better recommendations.",
      })
      return
    }
    ...
    setStatusMessage({
      type: "info",
      message: "Analyzing your profile and generating personalized project ideas...",
    })

    try {
      const generatedProjects = await generateProjectIdeas(resumeData, jobRole, industry, careerGoals)
      setProjects(generatedProjects)
      setActiveTab("all")
      setStatusMessage({
        type: "success",
        message: "Project ideas generated successfully! These are tailored to your skills and career goals.",
      })
    } catch (error) {
  ...
  const saveProject = (project) => {
    if (!savedProjects.some((p) => p.title === project.title)) {
      const updatedSavedProjects = [...savedProjects, { ...project, savedAt: new Date().toISOString() }]
  ...
  const removeProject = (projectTitle) => {
    const updatedSavedProjects = savedProjects.filter((p) => p.title !== projectTitle)
    setSavedProjects(updatedSavedProjects)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-blue-400">Career Details</CardTitle>
                <CardDescription className="text-gray-400">
                  Help us understand your career goals for better project recommendations
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <label className="text-sm text-gray-300 mb-1 block">Target Job Role *</label>
                  <Input
                ...
                <div>
                  <label className="text-sm text-gray-300 mb-1 block">Industry Focus</label>
                  <Input
                ...
                <div>
                  <label className="text-sm text-gray-300 mb-1 block">Career Goals (Optional)</label>
                  <Textarea
            ...
            {resumeData && (
              <Card className="bg-gray-800 border-gray-700">
                <CardHeader>
                  <CardTitle className="text-xl font-semibold text-blue-400">Your Profile</CardTitle>
                  <CardDescription className="text-gray-400">
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {resumeData.skills ? (
                        ...
                        <span className="text-gray-400 text-sm">No skills found in your resume</span>
                      )}
                    </div>
                  </div>
                  ...
                  {resumeData.title && (
                    <div>
                      <h3 className="text-gray-300 font-medium mb-1">Current Role</h3>
                      <p className="text-gray-400 text-sm">{resumeData.title}</p>
                    </div>
                  )}

                  {resumeData.bio && (
                    <div>
                      <h3 className="text-gray-300 font-medium mb-1">Summary</h3>
                      <p className="text-gray-400 text-sm line-clamp-3">{resumeData.bio}</p>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-blue-400">Trending Technologies</CardTitle>
                <CardDescription className="text-gray-400">
                    ...
                    {trendingTechnologies.map((tech, index) => (
                      <div key={index} className="p-3 bg-gray-700 rounded-md">
                        <div className="flex items-center justify-between mb-1">
                          <h3 className="text-gray-200 font-medium">{tech.name}</h3>
                          <div className="flex items-center">{getTrendIcon(tech.trend)}</div>
                        </div>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    <CardTitle className="text-xl font-semibold text-blue-400">Project Recommendations</CardTitle>
                    <CardDescription className="text-gray-400">
                      Personalized project ideas based on your skills and career goals
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent>
                {projects ? (
                              ...
                              <Card key={index} className="bg-gray-700 border-gray-600">
                                ...
                                <CardContent className="pt-0">
                                  ...
                                  <Accordion type="single" collapsible className="w-full">
                                    <AccordionItem value="tech-stack" className="border-gray-600">
                                      ...
                                      <AccordionContent>
                                        <div className="flex flex-wrap gap-2 py-2">
                                          {project.techStack.map((tech, i) => (
                                            <Badge key={i} className="bg-blue-900/40 text-blue-300 border-blue-700">
                                              {tech}
                                            </Badge>
                                          ))}
                                        </div>
                                    ...
                                    <AccordionItem value="learning-outcomes" className="border-gray-600">
                                      <AccordionTrigger className="text-sm text-gray-300 hover:text-gray-200 py-2">
                                        <div className="flex items-center">
                                    ...
                                    {project.implementationSteps && (
                                      <AccordionItem value="implementation" className="border-gray-600">
                                        <AccordionTrigger className="text-sm text-gray-300 hover:text-gray-200 py-2">
                                          <div className="flex items-center">
                                    ...
                                    {project.careerRelevance && (
                                      <AccordionItem value="career-relevance" className="border-gray-600">
                                        <AccordionTrigger className="text-sm text-gray-300 hover:text-gray-200 py-2">
                                          <div className="flex items-center">
                                        ...
                                        <AccordionContent>
                                          <p className="text-sm text-gray-300 py-2">{project.careerRelevance}</p>
                                        </AccordionContent>
                                      </AccordionItem>
                                    )}

                                    {project.similarProjects && (
                                      <AccordionItem value="similar-projects" className="border-gray-600">
                                        <AccordionTrigger className="text-sm text-gray-300 hover:text-gray-200 py-2">
                                          <div className="flex items-center">
                                        ...
                                        <AccordionContent>
                                          <ul className="py-2 space-y-2">
                                            {project.similarProjects.map((similar, i) => (
                                              <li key={i} className="text-sm">
                                                ...
                                                {similar.description && (
                                                  <p className="text-gray-400 text-xs mt-1">{similar.description}</p>
                                                )}
                                              </li>
                                            ))}
                                          </ul>
                            ...
                            <div className="text-center p-8 bg-gray-700 rounded-md">
                              {activeTab === "saved" ? (
                                <div className="flex flex-col items-center">
                                  ...
                                  <h3 className="text-gray-300 text-lg font-medium mb-2">No Saved Projects</h3>
                                  <p className="text-gray-400">
                                    You haven't saved any project ideas yet. Click the bookmark icon on projects you
                                    like to save them here.
                                  </p>
                                </div>
                              ) : (
                                <div className="flex flex-col items-center">
                                  <AlertCircle className="h-12 w-12 text-gray-500 mb-4" />
                                  <h3 className="text-gray-300 text-lg font-medium mb-2">No Projects Found</h3>
                                  <p className="text-gray-400">
                                    No {activeTab} level projects found. Try selecting a different difficulty level.
                                  </p>
                                </div>
                              )}
                            </div>
                  ...
                  <div className="bg-gray-700 rounded-md p-8 flex flex-col items-center justify-center min-h-[400px] text-center">
                    ...
                    <h3 className="text-gray-300 text-lg font-medium mb-2">No Project Ideas Generated Yet</h3>
                    <p className="text-gray-400 max-w-md">
                      Fill in your career details on the left and click "Generate Project Ideas" to get personalized
